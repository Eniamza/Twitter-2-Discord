...
const tweetrss = (client) => {
    ...
    rettiwt.tweet.search({
		...
		console.log(`New Tweet Detected! Link: https://twitter.com/DooggiesNFT/status/${newTweet.id}`)
		...
		// console.log(newTweet.media[0].url)
		// let tweetImage = newTweet.media[0].url

		// send_embed = tweetembed(tweetTitle,tweetLink,tweetImage)
		let sendLink = `https://twitter.com/DooggiesNFT/status/${newTweet.id}`

		try {

		const channel = client.channels.cache.get('932730922049110029');
        channel.send(sendLink);
